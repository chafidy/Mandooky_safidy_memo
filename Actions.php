...
Class Actions extends DBConnection{
    ...
    function login(){
        ...
        if(!$qry){
            $resp['status'] = "failed";
            $resp['msg'] = "Nom d'utilisateur ou mot de passe invalide.";
        }else{
            $resp['status'] = "success";
            $resp['msg'] = "Connexion réussi.";
            foreach($qry as $k => $v){
                if(!is_numeric($k))
                $_SESSION[$k] = $v;
            }
    ...
    function save_admin(){
        ...
        if(@$check> 0){
            $resp['status'] = 'failed';
            $resp['msg'] = "Nom d'utilisateur déjà existé.";
        }else{
            if(empty($id)){
            ...
            if($save){
                $resp['status'] = 'success';
                if(empty($id))
                $resp['msg'] = 'Nouveau utilisateur bien enregistré.';
                else
                $resp['msg'] = 'Détails utilisateur bien modifié.';
            }else{
                $resp['status'] = 'failed';
                $resp['msg'] = 'Enregistrement échoué. Error: '.$this->lastErrorMsg();
                $resp['sql'] =$sql;
            }
        }
        return json_encode($resp);
    ...
    function delete_admin(){
        ...
        if($delete){
            ...
            $_SESSION['flashdata']['msg'] = 'utlisateur bien supprimé.';
        }else{
            $resp['status']='failed';
    ...
    function user_register(){
        ...
        if(@$check> 0){
            $resp['status'] = 'failed';
            $resp['msg'] = "Nom d'utlisateur déjà existé.";
        }else{
            if(empty($id)){
            ...
            if($save){
                $resp['status'] = 'success';
                if(empty($id))
                $resp['msg'] = 'Compte bien créé.';
                else
                $resp['msg'] = 'Compte bien modifié.';
            }else{
                $resp['status'] = 'failed';
                $resp['msg'] = 'Création de compte échoué. Error: '.$this->lastErrorMsg();
                $resp['sql'] =$sql;
            }
        }
        return json_encode($resp);
    ...
    function update_credentials(){
        ...
        if(!empty($password) && md5($old_password) != $_SESSION['password']){
            $resp['status'] = 'failed';
            $resp['msg'] = "Ancien mot de passe incorrect.";
        }else{
            ...
            if($save){
                $resp['status'] = 'success';
                $_SESSION['flashdata']['type'] = 'success';
                $_SESSION['flashdata']['msg'] = 'Information bien modifié.';
                foreach($_POST as $k => $v){
                    if(!in_array($k,array('id','old_password')) && !empty($v)){
                        if(!empty($data)) $data .= ",";
                        if($k == 'password') $v = md5($v);
                ...
                $resp['status'] = 'failed';
                $resp['msg'] = 'Modification information échoué. Error: '.$this->lastErrorMsg();
                $resp['sql'] =$sql;
            }
        }
        return json_encode($resp);
    ...
    function save_category(){
        ...
        if(@$check> 0){
            $resp['status'] ='failed';
            $resp['msg'] = 'Cette catégorie est déjà existé.';
        }else{
            @$save = $this->query($sql);
            if($save){
                $resp['status']="success";
                if(empty($id))
                    $resp['msg'] = "Catégorie bien enregistré.";
                else
                    $resp['msg'] = "Catégorie bien modifié.";
            }else{
                $resp['status']="failed";
                if(empty($id))
                    $resp['msg'] = "Enregistrement nouvelle catégorie échoué.";
                else
                    $resp['msg'] = "Modification catégorie échoué.";
                $resp['error']=$this->lastErrorMsg();
                $resp['sql']=$sql;
            }
        }
    ...
    function delete_category(){
        ...
        if($delete){
            $resp['status']='success';
            $_SESSION['flashdata']['type'] = 'success';
            $_SESSION['flashdata']['msg'] = 'Catégorie bien supprimé.';
        }else{
            $resp['status']='failed';
            $resp['error']=$this->lastErrorMsg();
        }
    ...
    function save_recipe(){
        ...
        if($save){
            $resp['status']="success";
            if(empty($id))
                $resp['msg'] = "Recette bien ajoutée.";
            else
                $resp['msg'] = "Recette bien modifiée.";
            if(empty($id)){
            ...
            if(isset($_FILES['image']) && !empty($_FILES['image']['tmp_name'])){
                ...
                if(!in_array($type,array('image/png','image/jpeg'))){
                    $resp['msg'] = "Recette bien ajouté mais l'image téléversée est échoué due à la type de fihier invalide";
                }else{
                    ...
                    if($gdImage){
                        ...
                        imagedestroy($gdImage);
                            ...
                            $resp['msg'] = "Recette bien ajouté mais l'image téléversée est échoué due à la raison incorrect";
                    }
                }
            ...
            $resp['status']="failed";
            if(empty($id))
                $resp['msg'] = "Ajout de recette échoué.";
            else
                $resp['msg'] = "Modification de recette échoué.";
            $resp['error']=$this->lastErrorMsg();
        }
        return json_encode($resp);
    }
    function delete_recipe(){
        ...
        if($delete){
            $resp['status']='success';
            $_SESSION['flashdata']['type'] = 'success';
            $_SESSION['flashdata']['msg'] = 'Recette bien supprimée.';
    ...
    function save_comment(){
        ...
        if($save){
            ...
            if(empty($id)){
                $resp['msg'] = "Commentaire bien ajouté.";
            }else{
                $resp['msg'] = "Commentaire bien modifié.";
            }
            $_SESSION['comment_flashdata']['msg']=$resp['msg'];
        }else{
            $resp['status']="failed";
            if(empty($id))
                $resp['msg'] = "Ajout commentaire échoué.";
            else
                $resp['msg'] = "Modification commentaire échoué.";
            $resp['error']=$this->lastErrorMsg();
        }
        return json_encode($resp);
    }
    function delete_comment(){
        ...
        if($delete){
            $resp['status']='success';
            $_SESSION['comment_flashdata']['type'] = 'success';
            $_SESSION['comment_flashdata']['msg'] = 'Commentaire bien supprimé.';
        }else{
            $resp['status']='failed';
            $resp['error']=$this->lastErrorMsg();
        }
    ...
    function save_settings(){
        ...
        $resp['status'] = "success";
        $resp['msg'] = "Paramètre bien supprimé.";
        return json_encode($resp);
    }
}
